...
def _track_with_segments(n=4):
    return {
        ...
        "segment_timeline": [
            {"number": i + 1, "time": i * 1000, "duration": 1000, "timescale": 1000} for i in range(n)
        ],
        "id": "1",
    }
...
def test_download_single_file_fallback(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(dl, "_download_with_progress", lambda u, h, o, l: 123)
...
def test_download_track_resume_skips_cached_parts(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(dl, "_download_resource", lambda u, h, o, l: (Path(o).write_bytes(b"init"), 4)[1])
    ...
    asyncio.run(dl.download_track(track, None, "https://example.com/manifest.mpd", {}, out, "label", tmp_path))
...
def test_download_segments_rescue_raises_after_failures(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(dl, "_download_resource", lambda *a, **kw: (_ for _ in ()).throw(RuntimeError("fail")))
    ...
    async def run():
        ...
        try:
            await dl._download_segments(ex, segments, [0], "https://cdn/", "seg-$Number$.m4s", {}, parts_dir, "label")
        finally:
